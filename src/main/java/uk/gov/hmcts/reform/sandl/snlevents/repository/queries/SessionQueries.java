...
public final class SessionQueries {
    ...
    public static final String GET_SESSION_AMEND_RESPONSE_SQL =
        ...
        + "  (SELECT CASE WHEN EXISTS((SELECT hearing_part.id "
        + "                            FROM hearing_part "
        + "                            WHERE hearing_part.session_id = s.id AND "
        + "                                  hearing_part.is_deleted = FALSE AND hearing_part.status = 'Listed')) "
        + "    THEN TRUE "
        + "          ELSE FALSE END)                                           AS has_listed_hearing_parts "
        + "FROM Session s "

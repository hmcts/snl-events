...
public class SearchSessionQuery {
    ...
    private static final String SEARCH_SESSION_QUERY = "SELECT\n"
        ...
        + "  allocated_duration,\n"
        + "  utilisation,\n"
        + "  CASE WHEN utilisation > 100 THEN 0\n"
        + "  ELSE session_duration - allocated_duration END AS available\n"
        + "FROM (\n"
        + "       SELECT\n"
        + "         *, ROUND((allocated_duration / session_duration) * 100, 2) AS utilisation\n"
        + "       FROM  (\n"
        + "       SELECT *, CASE WHEN duration_sum IS NULL THEN 0 ELSE duration_sum END AS allocated_duration FROM (\n"
        + "              SELECT\n"
        + "                p.id                                 AS person_id,\n"
        ...
        + "                (SELECT count(id)\n"
        + "                 FROM hearing_part\n"
        + "                 WHERE session_id = main_session.id AND\n"
        + "                     hearing_part.is_deleted = FALSE) AS no_of_hearing_parts,\n"
        + "\n"
        + "                (SELECT SUM(\n"
        + "                    CASE WHEN h.is_multisession = TRUE\n"
        + "                  THEN\n"
        ...
        + "                   LEFT JOIN status_config sc on hearing_part.status = sc.status\n"
        + "                 WHERE\n"
        + "                   s.id = main_session.id AND\n"
        + "                   hearing_part.is_deleted = FALSE AND\n"
        + "                   sc.count_in_utilization = TRUE) * 1000000000 AS duration_sum\n"
        + "\n"
